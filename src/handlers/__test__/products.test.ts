...
describe('POST /api/products', () => {
    ...
    it('should display validations error', async () => {
        ...
        expect(response.body.errors).not.toEqual(3)
    })

    it('should validate that the price is grater than 0', async () => {
        const response = await request(server).post('/api/products')
            .send({
                name: "Test error price",
                price: 0
            })
...
describe('GET /api/products', () => {
    it('should ckeck if api/products url exists', async () => {
        const response = await request(server).get('/api/products')

        expect(response.status).not.toEqual(404)
    })

    it('GET a JSON response with proucts', async () => {
        const response = await request(server).get('/api/products')
...
describe('DELETE /api/products', () => {
    ...
    it('should delete a product', async () => {
        ...
        expect(response.status).not.toEqual(404)
        expect(response.body).not.toEqual('errors')
